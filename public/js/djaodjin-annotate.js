...
!!!NB: Adapted/Forked from the above by Jean-Louis Leysens!!!
*/
  ...
  Annotate.prototype = {
    init: function() {
      ...
      if (self.options.images) {
        ...
        self.baseCanvas.height = self.drawingCanvas.height =
          self.options.height;
      }
      ...
      $(document).on(
        self.options.selectEvent,
        '.annotate-image-select',
        function(event) {
          event.preventDefault();
          var image = self.selectBackgroundImage(
            $(this).attr(self.options.idAttribute)
          );
    ...
    setBackgroundImage: function(image) {
      ...
      self.img = new Image(500, 500);
      // self.img.setAttribute('crossOrigin', 'anonymous');
      self.img.src = image.path;
      self.img.onload = function() {
        if (
          (self.options.width && self.options.height) !== undefined ||
    ...
    checkUndoRedo: function() {
      var self = this;
      self.$tool
        .children('.annotate-redo')
        .attr('disabled', self.storedUndo.length === 0);
      self.$tool
        .children('.annotate-undo')
        .attr('disabled', self.storedElement.length === 0);
    },
    ...
    annotatemove: function(event) {
      ...
      switch (self.options.type) {
        case 'rectangle':
          self.clear();
          self.tox =
            (pageX - offset.left) * self.compensationWidthRate - self.fromx;
          self.toy =
            (pageY - offset.top) * self.compensationWidthRate - self.fromy;
          self.drawRectangle(
            self.drawingContext,
            self.fromx,
            self.fromy,
    ...
    annotateresize: function() {
      ...
      self.fontsize =
        String(
          parseInt(self.options.fontsize.split('px')[0], 10) *
            self.compensationWidthRate
        ) + 'px';
  ...
  $.fn.annotate = function(options, cmdOption, callback) {
    ...
    if (options === 'destroy') {
      if ($annotate) {
        ...
        throw new Error('No annotate initialized for: #' + $(this).attr('id'));
      }
